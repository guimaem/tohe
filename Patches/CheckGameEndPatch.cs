...
class GameEndChecker
{
        ...
        if (CustomWinnerHolder.WinnerTeam != CustomWinner.Default)
        {
            ...
            foreach (var pc in Main.AllPlayerControls)
            {
                ...
                switch (CustomWinnerHolder.WinnerTeam)
                {
                    ...
                    case CustomWinner.Jackal:
                        ...
                        break;
                    case CustomWinner.Briber:
                        if (pc.Is(CustomRoles.SidekickB) && !CustomWinnerHolder.WinnerIds.Contains(pc.PlayerId))
                        {
                            CustomWinnerHolder.WinnerIds.Add(pc.PlayerId);
                        }
                        break;
            ...
            if (CustomWinnerHolder.WinnerTeam is not CustomWinner.Draw and not CustomWinner.None and not CustomWinner.Error)
            {
                ...
                else if (CustomRoles.Lovers.RoleExist() && !reason.Equals(GameOverReason.HumansByTask))
                {
                    ...
                    {
                        if (CustomWinnerHolder.WinnerTeam is CustomWinner.Crewmate or CustomWinner.Impostor or CustomWinner.Briber or CustomWinner.Jackal or CustomWinner.Pelican)
                        {
                ...
                foreach (var pc in Main.AllPlayerControls)
                {
                    ...
                    // Exploiter
                    if (pc.Is(CustomRoles.Exploiter) && pc.IsAlive())
                    {
                        CustomWinnerHolder.WinnerIds.Add(pc.PlayerId);
                        if (CustomWinnerHolder.WinnerTeam != CustomWinner.Lovers) CustomWinnerHolder.AdditionalWinnerTeams.Add(AdditionalWinners.Exploiter);
                    }
                    //Witch
    ...
    public static void StartEndGame(GameOverReason reason)
    {
        ...
        foreach (var pc in Main.AllPlayerControls)
        {
            ...
            void SetGhostRole(bool ToGhostImpostor)
            {
                ...
                if (ToGhostImpostor)
                {
                    Logger.Info($"{pc.GetNameWithRole()}: changed to Impostor Ghost", "ResetRoleAndEndGame");
                ...
                {
                    Logger.Info($"{pc.GetNameWithRole()}: changed to Crewmate Ghost", "ResetRoleAndEndGame");
                    sender.StartRpc(pc.NetId, RpcCalls.SetRole)
    ...
    class NormalGameEndPredicate : GameEndPredicate
    {
        ...
        public bool CheckGameEndByLivingPlayers(out GameOverReason reason)
        {
            ...
            var apcArray = Main.AllAlivePlayerControls.ToArray();
            int dual = 0, impCount = 0, crewCount = 0;

            foreach (var pc in apcArray)
            {
                if (pc == null) continue;
