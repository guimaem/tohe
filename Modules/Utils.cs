...
public static class Utils
{
        ...
        try
        {
            ...
            switch (role)
            {
                ...
                case CustomRoles.Vulture:
                    ...
                    break;
                case CustomRoles.Venter when Venter.HasSkillLimit.GetBool():
                    ProgressText.Append(Venter.GetSkillLimit(playerId));
                    break;
                case CustomRoles.LimitedKiller:
		    ProgressText.Append(LimitedKiller.GetKillLimit(playerId));
		    break;
                case CustomRoles.Masochist:
    ...
    public static void ShowAllActiveSettings(byte PlayerId = byte.MaxValue)
    {
        ...
        foreach (var role in Options.CustomRoleCounts.ToArray())
        {
        ...
        foreach (var opt in OptionItem.AllOptions.Where(x => x.GetBool() && x.Parent == null && x.Id >= 80000 && !x.IsHiddenOn(Options.CurrentGameMode)).ToArray())
        {
        ...
        else
        {
            ...
            if (player.AmOwner)
            {
                if (!player.IsModClient()) return;
                {
                    ...
                    //{
                        //if (TranslationController.Instance.currentLanguage.languagID == SupportedLangs.Brazilian)
    ...
    public static void NotifyRoles(bool isForMeeting = false, PlayerControl SpecifySeer = null, bool NoCache = false, bool ForceLoop = true, bool CamouflageIsForMeeting = false, bool MushroomMixupIsActive = false)
    {
        ...
        logger.Info("NotifyRoles was called from " + callerClassName + "." + callerMethodName + "から呼び出されました");
    ...
    public static void CountAlivePlayers(bool sendLog = false)
    {
        ...
        {
            Logger.Info("Number Impostor left: " + AliveImpostorCount + "人", "CountAliveImpostors");
