...
// TOHE+
// https://github.com/Gurge44/TOHE_PLUS/blob/main/Roles/Impostor/Stealth.cs
public static class Stealth
{
    private static readonly int Id = 15130;
    ...
    public static void SetupCustomOption()
    {
        ...
        optionDarkenDuration = FloatOptionItem.Create(Id + 11, "StealthDarkenDuration", new(0.5f, 10f, 0.5f), 3f, TabGroup.ImpostorRoles, false)
            .SetParent(Options.CustomRoleSpawnChances[CustomRoles.Stealth])
            .SetValueFormat(OptionFormat.Seconds);
        KillCooldown = FloatOptionItem.Create(Id + 12, "KillCooldown", new(5f, 180f, 2.5f), 20f, TabGroup.ImpostorRoles, false)
            .SetParent(Options.CustomRoleSpawnChances[CustomRoles.Stealth])
            .SetValueFormat(OptionFormat.Seconds);
    }
    ...
    public static void SetKillCooldown(byte id) => Main.AllPlayerKillCooldown[id] = KillCooldown.GetFloat(); //Options.DefaultKillCooldown
    ...
    {
        if (!IsEnable) return;
        MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.StealthSync, SendOption.Reliable, -1);
    ...
    public static string GetSuffix(PlayerControl seer, PlayerControl target = null, bool isForMeeting = false, bool isHUD = false)
    {
        if (!IsEnable) return string.Empty;
        target ??= seer;
        // During the meeting, unless it's my suffix or it's dark everywhere, I won't show anything.
        return isForMeeting || seer != Utils.GetPlayerById(playerIdList[0]) || target != Utils.GetPlayerById(playerIdList[0]) || !darkenedRoom.HasValue || (seer.IsModClient() && !isHUD)
            ? string.Empty
